...
import java.sql.*;
public class PinChecking implements ActionListener{
    ...
    Font myFont;
    String ac;
    PinChecking(String AccountNumbers)
    { ac=AccountNumbers;
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if(e.getSource()==insertButton)
        {
            try{
                ...
                String password = "";
                String query= "SELECT PinNumber FROM Atm_users WHERE AccountNumber ="+"'"+ac+"'"; // query to be run
                Class.forName("com.mysql.cj.jdbc.Driver"); // Driver name
                Connection con = DriverManager.getConnection(url,username,password);
                Statement st = con.createStatement();
                ResultSet rs= st.executeQuery(query);
                rs.next();
                if(pin.equals(rs.getString("PinNumber"))) // Execute query
                {
                    //rs.getInt(query);
                    System.out.println("Pin Connected to your account!!");
                    frame.dispose();
                    new EnterIn(ac,pin);
                }
                else
                {
                    text.setText("");    
                    JOptionPane.showMessageDialog(frame, "Incorrect pin", "Wrong pin",JOptionPane.ERROR_MESSAGE);
